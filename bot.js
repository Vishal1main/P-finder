...
bot.command('poster', async (ctx) => {
  if (ctx.chat.type === 'private') {
    return ctx.reply('<b>❌ The /poster command only works in groups.</b>');
  }

  if (!ALLOWED_GROUPS.includes(ctx.chat.id)) {
    return ctx.reply('❌ This group is not authorized to use the /poster command.\n\nContact admin for group link</b>');
  }

  const queryText = ctx.message.text.split(' ').slice(1).join(' ').trim();
  if (!queryText) return ctx.reply('<b>Please provide a movie name. Example: /poster Jawan 2023</b>');
  ...
  try {
    ...
    if (!filtered) return ctx.reply('<b>Movie/TV Show not found or posters unavailable.</b>');
